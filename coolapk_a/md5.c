...
#define inlined BOOST_FORCEINLINE

inlined MD5_u32plus BOOST_UUID_DETAIL_MD5_F(MD5_u32plus x, MD5_u32plus y, MD5_u32plus z) {
    return ((z) ^ ((x) & ((y) ^ (z))));
}
inlined MD5_u32plus BOOST_UUID_DETAIL_MD5_G(MD5_u32plus x, MD5_u32plus y, MD5_u32plus z) {
    return ((y) ^ ((z) & ((x) ^ (y))));
}
inlined MD5_u32plus BOOST_UUID_DETAIL_MD5_H(MD5_u32plus x, MD5_u32plus y, MD5_u32plus z) {
    return (((x) ^ (y)) ^ (z));
}
inlined MD5_u32plus BOOST_UUID_DETAIL_MD5_H2(MD5_u32plus x, MD5_u32plus y, MD5_u32plus z) {
    return ((x) ^ ((y) ^ (z)));
}
inlined MD5_u32plus BOOST_UUID_DETAIL_MD5_I(MD5_u32plus x, MD5_u32plus y, MD5_u32plus z) {
    return ((y) ^ ((x) | ~(z)));
}
...
void MU(MD5_CTX *ctx, const void *data, unsigned long size)
{
